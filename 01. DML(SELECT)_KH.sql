...
/*
    ...
    부여하고자하는 별칭에 띄어쓰기 혹은 특수문자가 포함될 경우 반드시 더블퀘테이션("")를 써야됨!!    
*/

-- EMPLOYEE 테이블에서 직원명(별칭:이름), 연봉(별칭:연봉(원)), 보너스포함연봉(별칭:총소득(원)) 조회
...
--FROM EMPLOYEE;
-- DISTINCT는 SELECT절에 딱 한번만 기술할 수 있음
...
-- 급여가 400만원이상이고 직급코드가 'J2'인 사원의 모든 컬럼 조회
SELECT *
FROM EMPLOYEE
...
-- 입사일이 '90/01/01' ~ '01/01/01'인 사원의 모든 컬럼 조회
SELECT *
FROM EMPLOYEE
WHERE HIRE_DATE BETWEEN '90/01/01' AND '01/01/01';

-- 반대로 저사이의 범위에 입사한 사원들이 아닌 그 외에 입사한 사원
SELECT *
FROM EMPLOYEE
...
/*
    < LIKE >
    비교하려는 컬럼값이 내가 지정한 특정 패턴에 만족될경우 조회
...
--> 어떤게 와일드 카드로 어떤게 데이터값인지 구분지어줘야됨
--> 데이터로 처리할 값 앞에 임의로 나만의 와일드 카드를 제시하고 나만의 와일드카드를 ESCAPE OPTION 등록
SELECT EMP_ID, EMP_NAME, EMAIL
FROM EMPLOYEE
...
/*
    ...
    비교대상컬럼명 IN ('값', '값', '값', ...)
*/
-- D6 부서원들과 D8부서원들, D5 부서원들의 이름, 부서코드, 급여 조회
SELECT EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
...
/*
    < 연결 연산자 : || >
    여러 컬럼값들을 하나의 컬럼인것처럼 연결하거나, 컬럼과 리터럴을 연결할 수도 있다.
*/
...
*/
-- OR보다 AND가 먼저 실행
-- 직급코드가 J7 또는 J2 직급인 사원들 중 급여가 200만원이상인 사원들의 모든 컬럼 조회
SELECT *
FROM EMPLOYEE
...
/*
    ...
    SELECT문 가장 마지막 작성 뿐만 아니라 실행순서 또한 가장 마지막
...
-- BONUS 내림차순 정렬 (단, BONUS 값이 일치할 경우 그 때는 SALARY가지고 오름차순정렬)
SELECT * 
FROM EMPLOYEE
